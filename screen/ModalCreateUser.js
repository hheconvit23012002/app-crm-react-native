...
const ModalCreateUser = ({ visible, onClose, fetchUsers }) => {
    ...
    const submitUser = async () => {
        try {
            ...
            let data = await user.post('/register',formData)
            ...
            console.log("okok")
            onClose();
            showToast('error','add user error')
        }
        setFormData({
            email: '',
            password: '',
                    ...
                    <TouchableWithoutFeedback>
                        <View style={styles.modalContent}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                                <Text style={{ fontSize: 20, fontWeight: '700' }}>Add User</Text>
